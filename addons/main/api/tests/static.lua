...
function Tests.GetQualityColor()
	AreEqual({Addon.Static:GetQualityColor(4)}, {GetItemQualityColor(3)})
	AreEqual({Addon.Static:GetQualityColor(0)}, {1, .1, .1, 'ffff2020'})
end

function Tests:GetTypeIcon()
	local actual = Addon.Static:GetTypeIcon(6)
...
function Tests:GetBreedIcon()
	AreEqual(Addon.Static:GetBreedIcon(nil, 1), '')
	AreEqual(Addon.Static:GetBreedIcon(1, .8), '')
	AreEqual(Addon.Static:GetBreedIcon(3, 1), '|TInterface\\PetBattles\\PetBattle-StatIcons:22:22:0:0:32:32:16:32:0:16|t')
end
...
function Tests:KeepShort()
	...
	short = Addon.Static:KeepShort(short)
	long = Addon.Static:KeepShort(long)
...
function Tests:Date()
	local function test(day, month, year)
		...
		AreEqual({day, month, year}, {Addon.Static:UnpackDate(Addon.Static:GetDate())})
	end
